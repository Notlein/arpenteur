...
//   Inversion des points en x de 0 -> (max - min) | (max-min) -> 0
  ...
  function initiate() {
    ...
    let ecrit = setInterval(function () {
      ...
      // 0&1, 1&2, 2&3, ... n-1&n
      let y = (Math.floor(listeDePoints[l - 1][1] / ratio)*1.3 + OFFSET_Y)/ devicePixelRatio;
      let x = (Math.floor(listeDePoints[l - 1][0] / ratio) + OFFSET_X)/ devicePixelRatio;;
      let nxt_y = (Math.floor(listeDePoints[l][1] / ratio) *1.3+ OFFSET_Y)/ devicePixelRatio;;
      let nxt_x = (Math.floor(listeDePoints[l][0] / ratio) + OFFSET_X)/ devicePixelRatio;;
...
function carousel(){
  ...
  var car = setInterval(function(){
    ...
    if (currentNDX >= getNbJourInstall()*2) {
      createNewMixLoop();
    setTimeout(clickButton, 2000);
      clearInterval(car);
