 ...
 */
tcxParser.prototype.parse = function (gpxstring) {
    let keepThis = this;
    ...
    var trks = [].slice.call(this.xmlSource.querySelectorAll('track'));
    ...
    for (let idx in trks){
        ...
        let trackpoints = [];
        let trkpts = [].slice.call(trk.querySelectorAll('trackpoint'));
	    for (let idxIn in trkpts){
            var trkpt = trkpts[idxIn];
            let pt = {};
            
            pt.lat = keepThis.getElementValue(trkpt,"latitudedegrees");
            pt.lon = keepThis.getElementValue(trkpt,"longitudedegrees");
