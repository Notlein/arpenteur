...
$("#overlay-droit").on("click",function () {
  if (right == 1 && currentNDX < (getNbJourInstall() * 2)+arr_data.length) {
    createNewMixFwd();
...
function initDisto(){
  let w = $(window).width();
let v = $(window).width()/1920; // ratio to 1080p img +"");
  $(".distortion").css("width", w +"");
  $(".distortion").css("height", w/16*9 +"");
}
...
$(document).ready(function(){
  ...
  setTimeout(function(){traceAll()},1500);
  
  setTimeout(function(){$("#splashload").fadeOut(2000)}, 500);
