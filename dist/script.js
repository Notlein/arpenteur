...
$("#overlay-droit").on("click",function () {
  if (currentNDX < img_Array.length) {
...
function openMenu(){
  $("#splash").fadeIn();
  $("#fs-icone, #trace-icone, .phrase-explicative").fadeOut();
}

function closeMenu(){
  $("#splash").fadeOut();
  $("#fs-icone, #trace-icone, .phrase-explicative").fadeIn();
}
  ...
  function initiate() {
    ...
    let ecrit = setInterval(function () {
      if (l + 1 >= listeDePoints.length) {
        clearInterval(ecrit);
        // setTimeout(document.querySelector("#trace-icone").click() ,10000);
        // 
        // Désactivé car nous jugeons qu'il est plus intéressant de conserver le tracé à la fin, sinon celui-ci s'efface sans qu'on puisse l'attraper
        //
...
$(document).ready(function(){
  ...
  setTimeout(function(){
      document.querySelector('#overlay-droit').click();
